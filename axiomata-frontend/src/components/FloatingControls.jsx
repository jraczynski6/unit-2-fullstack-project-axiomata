...
export default function FloatingControls({
  ...
  const handleConfirmDeleteEntity = async () => {
    ...
    try {
      switch (selectedEntity.category) {
        case "Location": await deleteLocation(selectedEntity.id); break;
        case "Faction": await deleteFaction(selectedEntity.id); break;
        case "Character": await deleteCharacter(selectedEntity.id); break;
        case "Item": await deleteItem(selectedEntity.id); break;
        default: throw new Error("Unknown category");
      }

      onDeleteEntity?.(selectedEntity); // update local state
    } catch (err) {
      console.error(err);
  ...
  const handleDeleteClick = () => {
    console.log("Delete button clicked, selectedEntity:", selectedEntity);
    setConfirmOpen(true);
  };
